...
const { mediaItems, mediaItemMetadata, mediaItemInstances } = await import(
	"./schema.ts"
);
...
async function seed() {
	...
	await db.delete(mediaItemMetadata);
	await db.delete(mediaItems);
		...
		.values([
			{
				...
				description:
					"A young man grows up to be the most notorious wizard his world has ever seen.",
	...
	const [breakingBad, lastOfUs, severance] = await db
		...
		.values([
			...
			},
			{
				...
				description:
					"A smuggler and a teenage girl traverse a post-apocalyptic United States.",
				coverImageUrl:
			...
			},
			{
				...
				title: "Severance",
				description:
					"A company offers a radical work-life balance by surgically dividing employees' memories.",
				coverImageUrl:
		...
		.values([
			{
				...
				title: "Hollow Knight",
				description:
					"A challenging action-adventure game set in a vast underground kingdom of insects.",
				coverImageUrl:
			...
			},
			{
				...
				title: "Elden Ring",
				description:
					"An action RPG set in a vast world with deep lore and punishing combat.",
				coverImageUrl:
			...
			},
			{
				...
				description:
					"An epic RPG set in the Forgotten Realms with deep choices and consequences.",
				coverImageUrl:
					"https://images.igdb.com/igdb/image/upload/t_cover_big/co6li5.jpg",
	...
	await db.insert(mediaItemInstances).values([
		...
		{
			mediaItemId: itemNameOfTheWind.id,
			rating: "9.0",
			reviewText:
				"One of the best fantasy novels I've ever read. Kvothe's voice is incredible.",
			startedAt: "2023-01-10",
			completedAt: "2023-02-01",
		},
		// Dune — first read completed, re-read currently in progress
		{
			mediaItemId: itemDune.id,
			rating: "9.5",
		...
		{
			mediaItemId: itemOppenheimer.id,
			rating: "8.5",
			reviewText:
				"Stunning cinematography and a gripping story. Cillian Murphy is phenomenal.",
			startedAt: "2023-07-22",
			completedAt: "2023-07-22",
		},
		// Breaking Bad — one completed watch
		{
			mediaItemId: itemBreakingBad.id,
			rating: "9.5",
			reviewText:
				"The greatest TV show ever made. Walter White's arc is unrivaled.",
			startedAt: "2021-09-01",
			completedAt: "2021-11-30",
		},
		// The Last of Us — currently watching, no rating yet
		{
			mediaItemId: itemLastOfUs.id,
			rating: null,
			reviewText: null,
		...
		{
			mediaItemId: itemHollowKnight.id,
			rating: "9.0",
			reviewText:
				"Brutally difficult but incredibly rewarding. The atmosphere is unmatched.",
			startedAt: "2023-05-01",
			completedAt: "2023-06-20",
		},
		// Elden Ring — currently playing
		{
			mediaItemId: itemEldenRing.id,
			rating: null,
			reviewText: null,
