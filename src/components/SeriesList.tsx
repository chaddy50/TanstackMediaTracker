...
import { NextItemStatusBadge } from "#/components/NextItemStatusBadge";
import { SeriesCompletionBadge } from "#/components/SeriesCompletionBadge";
...
export function SeriesList({ items }: SeriesListProps) {
	...
	const hasAnyNextItemStatus = items.some(
		(item) => item.nextItemStatus !== null,
	);
	const thClass =
		"md:sticky md:top-[65px] z-10 bg-background border-b border-border px-2 py-2 text-left text-xs text-muted-foreground font-medium uppercase tracking-wide whitespace-nowrap";
